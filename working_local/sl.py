...
def get_first_7_words(sentence):
    ...
    words_res = first_7_words.rstrip()
    # return words_res
    words_res= words_res.split()
    first_7_words=""
    for i,word in enumerate(words_res[::-1]):
        first_7_words+=' '+word
...
def generate_thumbnail_caption(video_description):
    ...
    thumbnail_caption_2 = ""#text_generator(video_description, max_length=200, num_return_sequences=1)[0]['generated_text']

    parser = PlaintextParser.from_string(video_description, Tokenizer("english"))
    
    # Initialize the LSA summarizer
    summarizer = LsaSummarizer()
    
    # Summarize the text to extract the key points
    summary = summarizer(parser.document, 1)  # Adjust the number of sentences in the summary if needed
    
    # Get the theme/topic from the summary
    thumbnail_caption_3 = str(summary[0])

    doc_desc = nlp(video_description)

    doc_1 = nlp(thumbnail_caption_1)
    doc_2= nlp(thumbnail_caption_2)
    doc_3= nlp(thumbnail_caption_3)

    score_1=doc_desc.similarity(doc_1)
    score_2=doc_desc.similarity(doc_2)
    score_3=doc_desc.similarity(doc_3)
    
    print("thumbnail_caption_1:\t",thumbnail_caption_1,"\nscore:\t",score_1)
    print("thumbnail_caption_2:\t",thumbnail_caption_2,"\nscore:\t",score_2)
    print("thumbnail_caption_3:\t",thumbnail_caption_3,"\nscore:\t",score_3)
    ...
    return get_first_7_words(thumbnail_caption_3)
